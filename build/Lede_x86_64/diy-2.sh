...
# sed -i "/uci commit network/i\uci set network.lan.ipaddr='10.1.1.1'" $ZZZ                       # IPv4 地址(openwrt后台地址)
# sed -i "/uci commit network/i\uci set network.lan.netmask='255.255.255.0'" $ZZZ                 # IPv4 子网掩码
# sed -i "/uci commit network/i\uci set network.lan.gateway='10.1.1.1'" $ZZZ                      # IPv4 网关
# sed -i "/uci commit network/i\uci set network.lan.broadcast='10.1.1.255'" $ZZZ                  # IPv4 广播
...
sed -i "/uci commit system/i\uci set system.@system[0].hostname='OP'" $ZZZ                      # 修改主机名称为OP

sed -i "s/OpenWrt /Derekmini build $(TZ=UTC-8 date "+%Y.%m.%d") @ OpenWrt /g" $ZZZ              # 增加自己个性名称Derekmini

sed -i 's/$1$V4UetPzk$CYXluq4wUazHjmCDBCqXF.:0/$1$DvXOFLQQ$ReseTAbhIpWzBLwWU4hI80:18680/g' $ZZZ # 设置密码
